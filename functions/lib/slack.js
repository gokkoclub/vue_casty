            ...
            for (let attempt = 0; attempt < 3 && !ts; attempt++) {
                ...
                try {
                    ...
                    const byFileId = uploadedFileId
                        ? msgs.find((m) => Array.isArray(m.files) && m.files.some((f) => f.id === uploadedFileId))
                        : undefined;
...
function buildOrderMessage(params) {
    ...
    if (hasPerCastDates && hasMultipleDates) {
        ...
        const castDateMap = {}; // key: roleName_rank_castName
        const castInfoMap = {};
        ...
        for (const [key, info] of Object.entries(castInfoMap)) {
            if (!roleOrder[info.roleName])
                roleOrder[info.roleName] = [];
