    ...
    try {
        ...
        try {
            ...
            for (const doc of adminAll.docs) {
                const d = doc.data();
                const candidates = [d.name, d.displayName, d.slackName].filter((s) => !!s);
 ...
 */
async function findThreadTsByTokens(slackToken, channel, tokens, maxPages = 5) {
    const ids = (tokens || []).filter(Boolean);
    if (!slackToken || !channel || ids.length === 0)
        return null;
    let cursor;
    for (let page = 0; page < maxPages; page++) {
        ...
        if (hit) {
            const permalink = await (0, _helpers_1.getSlackPermalink)(slackToken, channel, hit.ts);
...
exports.notifyOrderCreated = (0, https_1.onCall)({
    ...
    const shootIds = data.projectId
        ? [...orderDateSet].sort().map((ymd) => (0, _helpers_1.buildShootId)(data.projectId, ymd)).filter(Boolean)
        : [];
    ...
    try {
        slackResult = data.pdfBase64 && data.pdfFileName
            ? await (0, slack_1.uploadFileToSlack)(slackToken, postChannel, message, data.pdfBase64, data.pdfFileName, threadTsForReply, data.castingIds || [])
            ...
            if (orderMode === "external") {
                try {
                    ...
                    if (Array.isArray(emails)) {
                        externalEmails = emails.filter((e) => typeof e === "string" && e.includes("@"));
                    }
                    ...
                    try {
                        ...
                        if (eventId && castingDocId) {
                            ...
                            for (const extEmail of externalEmails) {
                                if (extEmail.toLowerCase() === (castEmail || "").toLowerCase())
                                    continue;
                                const extRes = await (0, gasCalendar_1.addAttendeeViaGas)({
                                    webhookUrl: gasWebhookUrl,
                                    secret: gasSharedSecret,
                                    calendarId,
                                    eventId,
    ...
    if (threadTs && (orderMode === "shooting" || !orderMode)) {
        const internalItemsForDm = data.items.filter(item => item.castType === "内部" &&
            item.slackMentionId &&
...
exports.notifyStatusUpdate = (0, https_1.onCall)({
    ...
    if (!slackThreadTs && casting.projectId && slackToken && slackChannel) {
        try {
            const selfYmd = (() => { try {
                return casting.startDate?.toDate?.().toISOString().slice(0, 10) || "";
            }
            catch {
                return "";
            } })();
            const shootId = selfYmd ? (0, _helpers_1.buildShootId)(casting.projectId, selfYmd) : "";
            ...
            if (found) {
                ...
                const sibling = sibSnap.docs.find(d => {
                    const dd = d.data();
                    if (!(dd.slackThreadTs && dd.deleted !== true && dd.status !== "キャンセル" && dd.status !== "NG" && dd.status !== "削除済み"))
                        return false;
                    if (!selfYmd)
                        return true;
                    let sibYmd = "";
                    try {
                        sibYmd = dd.startDate?.toDate?.().toISOString().slice(0, 10) || "";
                    }
                    catch { /* noop */ }
...
exports.repairCastingThread = (0, https_1.onCall)({
    maxInstances: 10,
    secrets: [
        "SLACK_BOT_TOKEN",
        "SLACK_CHANNEL_INTERNAL",
    ...
    if (data.manualUrl) {
        ...
        const urlStr = String(data.manualUrl).trim();
        const urlMatch = urlStr.match(/\/archives\/(C[A-Z0-9]+)\/p(\d{10})(\d{6})/);
        if (!urlMatch) {
            throw new https_1.HttpsError("invalid-argument", "Slack URL の形式が不正です（…/archives/C.../p1234567890123456 を想定）");
        }
    ...
    if (!casting.projectId) {
        throw new https_1.HttpsError("failed-precondition", "casting に projectId がないため自動復旧不可");
    }
    if (!slackToken || !slackChannel) {
        throw new https_1.HttpsError("failed-precondition", "Slack認証情報がない");
    }
    const selfYmd = (() => { try {
        return casting.startDate?.toDate?.().toISOString().slice(0, 10) || "";
    }
    ...
    if (!slackThreadTs) {
        throw new https_1.HttpsError("not-found", "自動復旧できませんでした。Slack URLを手動で指定してください");
    }
    await castingDoc.ref.update({
        slackThreadTs,
        slackChannel,
 ...
 */
exports.sendPromotionDm = (0, https_1.onCall)({ secrets: ["SLACK_BOT_TOKEN", "GOOGLE_SERVICE_ACCOUNT_KEY"] }, async (request) => {
    const { castingId } = request.data;
    if (!castingId)
        throw new https_1.HttpsError("invalid-argument", "castingId is required");
    ...
    await (0, slack_1.sendDmToUser)(slackToken, slackMentionId, dmText, dmBlocks);
