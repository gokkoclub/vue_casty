...
exports.scheduledSyncFromNotion = exports.syncFromNotion = void 0;
/**
...
function readRichTextOrSelect(props, candidates) {
...
function readDateProp(props) {
    for (const key of PROP_DATE_CANDIDATES) {
        const p = props[key];
        if (p?.date?.start)
            return p.date.start.substring(0, 10); // YYYY-MM-DD
    }
    return "";
}
async function performSync() {
    ...
    if (!token || !databaseId) {
        throw new https_1.HttpsError("failed-precondition", "NOTION_TOKEN / NOTION_DATABASE_ID secret が未設定です");
    }
    ...
    do {
        ...
        for (const page of (resp.results || [])) {
            try {
                if (page.archived)
                    continue; // Notion 側でアーカイブされたら deleted 扱い（incomingNotionIds に入れない）
    ...
    console.log(`[syncFromNotion] synced=${synced} added=${added} updated=${updated} deletedMarked=${deletedMarked} restored=${restored} errors=${errors}`);
    return { synced, added, updated, deletedMarked, restored, errors };
}
/**
 * 手動実行（onCall）— UI から呼ぶ用
 */
exports.syncFromNotion = (0, https_1.onCall)({
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Notion sync failed: " + msg);
    }
});
/**
 ...
 * 動作確認後に有効化する。
 */
exports.scheduledSyncFromNotion = (0, scheduler_1.onSchedule)({
...
//# sourceMappingURL=syncFromNotion.js.map