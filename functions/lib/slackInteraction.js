        ...
        try {
            ...
            if (terminalStatuses.includes(oldStatus)) {
                ...
                if (dmChannel && messageTs) {
                    ...
                    await (0, slack_1.updateSlackMessage)(slackToken, dmChannel, messageTs, statusText.replace(/\*/g, ""), updatedBlocks);
                }
                return;
            }
            ...
            if (extraIds.length > 0) {
                try {
                    ...
                    for (const s of extraSnaps) {
                        if (!s.exists)
                            continue;
                        if (terminalStatuses.includes(s.data()?.status || ""))
                            continue;
                        batch.update(s.ref, {
                            status: "OK",
                            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            ...
            if (threadChannel && threadTs) {
                ...
                await (0, slack_1.postToSlack)(slackToken, threadChannel, replyText, undefined, threadTs);
