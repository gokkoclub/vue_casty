...
async function performOffshotFileCountSync(serviceAccountKey: string): Promise<SyncResult> {
    ...
    for (const doc of driveSnap.docs) {
        const data = doc.data();
        const driveLink: string | undefined = data.driveLink;
        const notionPageId: string | undefined = data.notionPageId;
    ...
    for (const contactDoc of contactsSnap.docs) {
        ...
        if (fileCount === undefined) continue;

        // 値が変わらないものは書き込まない
        if (contact.makingFileCount === fileCount) continue;
