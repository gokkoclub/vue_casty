...
export async function uploadFileToSlack(
    ...
    try {
        ...
        const getUrlResult = await getUrlResponse.json() as {
            ok: boolean;
            upload_url?: string;
            file_id?: string;
            error?: string;
        ...
        const completeResult = await completeResponse.json() as {
            ok: boolean;
            files?: Array<{ id: string; shares?: Record<string, Record<string, Array<{ ts: string }>>> }>;
            error?: string;
        };
        ...
        if (!ts) {
            ...
            const infoResult = await infoResponse.json() as {
                ok: boolean;
                file?: { shares?: Record<string, Record<string, Array<{ ts: string }>>> };
            };
